            ...
            loop {
                match connect_async(ws_url.as_str()).await {
                    Ok((ws_stream, _)) => {
                        ...
                        println!("\n✅ Flashblocks WebSocket connected successfully!");
                    ...
                    Err(e) => {
                        ...
                        if error_str.contains("Connection refused") {
                            eprintln!("\n⚠️  Flashblocks WebSocket connection refused");
                            ...
                            eprintln!("\n⚠️  Flashblocks WebSocket connection timed out");
                            eprintln!("    The server may be slow to respond or unreachable");
                            eprintln!("    Will retry in {:?}...", backoff);
                        } else {
                            eprintln!("\n⚠️  Flashblocks WebSocket error: {}", e);
                            eprintln!("    Will retry in {:?}...", backoff);
                        }
...
fn try_decode_message(bytes: &[u8]) -> eyre::Result<FlashBlock> {
    ...
    println!("\n📦 Raw Flashblock JSON (decompressed from binary):");
    println!("{}", text);
    println!("--- End of Raw Flashblock ---\n");
